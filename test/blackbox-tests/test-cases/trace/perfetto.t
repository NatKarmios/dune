...
instant carrying `dur_ns`, associated via the relevant rule or dep ID. The
`async_id` that paired the csexp begin with its end is converter-internal and
...
(on a finish or a collapsed instant) `dur_ns`. An executed rule's action
execution gets a lifecycle pair of its own; it is not bounded by -j, as an
arbitrary number of actions can be open at once.
...
not appear on them at all -- they are blob-only (see below). In particular,
"out.txt" (a `target_files` entry) no longer appears anywhere in the plain
protobuf text as its own interned string; it only shows up encoded inside the
blob's `data` payload. Its full path is still interned once, but as a
*user-requested* target on the `targets` event -- those are real paths in the
event, not intern ids, and are not part of the graph:
...
`forced_by` is blob-only for graph spans too, where its parts are intern ids,
but it does reach the trace on process slices, which have no blob to defer to
and so carry literal strings. It is a `dune` dict entry there, surfacing as
`debug.dune.forced_by`, and its parts are joined into one phrase rather than
indexed as an array. Recognised structural fields are grouped under that dict
(surfacing as e.g. `debug.dune.process_args` in Trace Processor):
...
usage) are top-level annotations rather than dict entries, as are
unrecognised fields such as the `config` event's own `build_dir`:
...
clears prior state (`sequence_flags: 3`), the rest only announce they need it
...
`_build/trace.csexp`. Converting the same events twice gives byte-identical
output -- the graph blob's dep-set factoring breaks its ties deterministically:
...
The matching graph-deps records (`<dep_id>~<resolution>~<forced_by>~<status>`)
resolve dep.txt to the rule that produces it ("r<rule_id>"), and the glob to
its expanded members, also by dict id rather than full paths ("x<id,id,...>").
The status field is empty: both deps were built successfully.
...
dep, and one that fails the same way but whose deps are hidden by the failure,
its dep being behind a `%{read:}` of the broken file (see `watch_memo` in
...
A failed rule keeps its begin/end pair and its real duration, so it converts
like an executed rule rather than collapsing:

  $ grep -q 'name: "exec-rule-finish"' dump.textpb && echo yes
  yes

The dep failure ("D") that reached its deps names the set holding them,
which resolves back to the file it was waiting for. Its build-dep record
carries the "f" status: the dep resolved to a rule, and that rule then failed.
  ...
  $ broken_dep_id=$(decode_section graph-dict \
  >   | grep '~_build/default/broken.txt$' | cut -d~ -f1)
  $ decode_section graph-deps | grep -E "^$broken_dep_id~r[0-9]+~" | cut -d~ -f4 \
  >   | sort -u
  f

The one whose deps were hidden says "?", which an empty field would
