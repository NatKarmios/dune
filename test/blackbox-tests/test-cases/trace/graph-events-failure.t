...
The outcome of each rule, keyed by the target it produces. The rules under
.dune are dune's own and are left out: whether they execute or hit a cache
depends on the state of the shared cache, not on this test.
...
it went. The two are independent: a.src and b.src resolve to rules and are
built, while boom.txt resolves to its rule and then fails -- before this the
failing dep was indistinguishable from a built one, since the resolution is all
the span carried.
