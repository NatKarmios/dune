...
module Graph_blob = struct
  ...
  module Dep_sets = struct
    (* Measured over a real monorepo trace rather than picked by taste. In
       particular a *larger* pool measured worse: stale cores win the
       "largest contained core" test with a poor covering, displacing the
       mining step that would have found a better one. *)
    let window_size = 32
    let pool_size = 64
    ...
    (* Ties go to the oldest candidate, so the result does not depend on
       anything but the order the sets arrived in. *)
...
(* The open spans of one kind, in a deterministic order: for the flushes at
   EOF. *)
let sorted tbl =
...
module Debug_annot = struct
  ...
  (* Massage particular fields, which would otherwise land as raw JSON. Graph
     events never come through here: their payloads are intern ids, resolved
     through the blob's dict rather than rendered. *)
  let classify ~name key v =
    ...
    | "forced_by", Sexp.List parts ->
      (* For process slices, whose [forced_by] parts are literal strings
         rather than the intern ids a graph event would carry. *)
...
type t =
  ...
  ; (* Timestamp (ns) of the last event seen, including [intern] events; used
       to close the spans that are still open at the end of the file, and to
       place the graph blob's instants. *)
    mutable last_ts : int
  ; open_rules : (Span_id.t, Span.Rule.t) Table.t
  ; open_deps : (Span_id.t, Span.Dep.t) Table.t
  ; open_gen_rules : (Span_id.t, Span.Gen_rules.t) Table.t
  ; open_dynamic_includes : (Span_id.t, Span.Dynamic_includes.t) Table.t
  ; open_actions : (Span_id.t, Span.Action.t) Table.t
  ; open_processes : (Span_id.t, Span.Process.t) Table.t
...
let ids key rest =
  ...
  | Some (List ids) -> Debug_annot.atoms ids
  | _ -> []
;;
    ...
    List.iter entries ~f:(fun entry ->
      ...
      | Some (Atom id), Some (Atom value) ->
        (match int_of_string_opt id with
         | Some id -> Table.set t.interned_strings id value
         | None -> ())
      | _ -> ())
  | _ -> ()
;;
...
module Graph_span = struct
  module Exec_rule = struct
    ...
    let record_end t ~span_id ~ts rest =
      ...
      | Some b ->
        ...
        let rule_outcome =
          ...
          | _ -> None
        in
        (* A missing [deps] is not "no deps": [deps_unknown] is what says dune
        ...
        let dyn_deps =
          ...
          | Some (List stages) ->
            List.map stages ~f:(function
              | Sexp.List stage_ids -> Debug_annot.atoms stage_ids
              | _ -> [])
          | _ -> []
        in
    ...
    let flush_unmatched t =
      List.iter (sorted t.open_rules) ~f:(fun (_span_id, b) ->
    ...
    module Action = struct
      ...
      let flush_unmatched t =
        List.iter (sorted t.open_actions) ~f:(fun (_span_id, b) -> push_start t b)
      ;;
    end
  end
  ...
  module Build_dep = struct
    ...
    let record_end t ~span_id ~ts rest =
      ...
      | Some b ->
        ...
        (* No [dep_status] means the dep was built successfully. *)
    ...
    let flush_unmatched t =
      List.iter (sorted t.open_deps) ~f:(fun (_span_id, b) ->
  ...
  module Gen_rules = struct
    ...
    let flush_unmatched t =
      List.iter (sorted t.open_gen_rules) ~f:(fun (_span_id, b) -> push_start t b)
    ;;
  end
  ...
  module Dynamic_includes = struct
    ...
    let flush_unmatched t =
      List.iter (sorted t.open_dynamic_includes) ~f:(fun (_span_id, b) -> push_start t b)
    ;;
  end
...
module Process = struct
  ...
  let flush_unmatched t =
    List.iter (sorted t.open_processes) ~f:(fun (_span_id, (b : Span.Process.t)) ->
