...
A `D` line carries the deps dune recovered for the rule without building
them, or `?` if it could not. An `A` line carries the deps it had resolved. A
`C` line carries them if the rule got that far, and `?` otherwise — a
cancelled rule is never made to recover them, since the build is going away.

`<forced_by>`: `r<rule_id>` | `v<rule_id>` (recovering that rule's deps after
it failed, i.e. outcome `D`) | `d<dep_id>` | `i<path_id>`
...
- `test/blackbox-tests/test-cases/trace/interrupted-watch-build-events.t` — a
  trace that can contain unfinished spans.
